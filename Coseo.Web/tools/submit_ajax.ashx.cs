    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void user_register(HttpContext context)
        {
            ...
            else
            {
                ...
                //防止Session提前过期
                Utils.WriteCookie(DTKeys.COOKIE_USER_NAME_REMEMBER, "Coseo", model.user_name);
                Utils.WriteCookie(DTKeys.COOKIE_USER_PWD_REMEMBER, "Coseo", model.password);
        ...
        private void user_login(HttpContext context)
        {
            ...
            {
                Utils.WriteCookie(DTKeys.COOKIE_USER_NAME_REMEMBER, "Coseo", model.user_name, 43200);
                Utils.WriteCookie(DTKeys.COOKIE_USER_PWD_REMEMBER, "Coseo", model.password, 43200);
            }
            else
            {
                //防止Session提前过期
        ...
        private void order_save(HttpContext context)
        {
            ...
            //检查购物车商品
            IList<Model.cart_items> iList = Coseo.Web.UI.ShopCart.GetList(user_group_id);
            if (iList == null)
            {
            ...
            //统计购物车
            Model.cart_total cartModel = Coseo.Web.UI.ShopCart.GetTotal(user_group_id);
            //保存订单=======================================================================
